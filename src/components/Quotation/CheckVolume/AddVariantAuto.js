...
const AddVariantAuto = ({
  ...
  const submitRequestHandler = async () => {
    ...
    if (error.length === 0) {
      const body = JSON.stringify({
        ...
        img_product:
          allVariant.pic_url.trim().length === 0
  ...
  const fetchData = async (url, tokenCookies) => {
    const body = JSON.stringify({ url });
    setLoading(true);
    if (url !== "") {
      try {
        const res = await fetch(
          `https://gateway.ocistok.co.id/oms/get-china-detail`,
          {
            headers: {
