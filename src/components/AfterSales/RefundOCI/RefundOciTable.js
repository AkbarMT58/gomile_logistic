...
import { getRefundOciData } from 'service/api';
import PaginationFilter from '../../General/PaginationFilter';

export default function RefundOciTable() {
  ...
  const fetchOrderData = async (limit, page) => {
    ...
    const data = await getRefundOciData(params);
    if (data?.status === 200) {
      const newFormat = [];
      ...
      <>
        <TableRow style={{ display: 'flex' }}>
          ...
          <TableCell
            ...
            <div className="space-y-2 mx-3"></div>
          </TableCell>
        </TableRow>
        <TableRow>
