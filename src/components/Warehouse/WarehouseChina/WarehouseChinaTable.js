...
import { getWarehouseChinaData } from "service/api";
import swal from "sweetalert";
import PaginationFilter from "../../General/PaginationFilter";

export default function WarehouseChinaTable() {
  ...
  const fetchOrderData = async (limit, page) => {
    ...
    const data = await getWarehouseChinaData(limit, page);
  ...
  const searchData = async (limit, page, search) => {
    const data = await getWarehouseChinaData(limit, page, search);
    if (data?.status === 200) {
      setDataOrder(data.data);
    }
    if (data?.status === 404) {
