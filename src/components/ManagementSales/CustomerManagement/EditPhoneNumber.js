...
import { updatePhoneNumber } from "service/api";
...
export default function EditPhoneNumber({
  ...
  const updatePhone = async () => {
    const res = /^(\+62|62)?[\s-]?0?8[1-9]{1}\d{1}[\s-]?\d{7,9}$/.test(input);
    setLoading(true);
    if (res) {
      const body = JSON.stringify({ email: email, phoneNumber: input });
      const data = await updatePhoneNumber(body);
      if (dataFromSearch) {
        setSearchData(`/${email}`);
      }
      if (data) {
